...
export default function ViewBillScreen({ route, navigation }) {
  ...
  const vatAmount = useMemo(
    () => Number(bill?.vat_amount ?? bill?.Vat_Amount ?? 0),
    [bill],
  );
  ...
  const billDiscount = useMemo(() => {
    ...
    return Math.max(combined - totalItemDiscount, 0);
  }, [bill, totalItemDiscount]);
  ...
  const shopPhone = useMemo(
    () => customerExtra?.Contact_1 || bill?.Customer_Phone || "",
    [customerExtra, bill],
  );
  const invoiceNo = bill?.Invoice_no || bill?.Sales_no || "N/A";

  // Helper for return calculations
    ...
    <body>
      ...
      <div class="totals-area">
        <table style="width:100%;">
          ...
          ${totalReturnAmount > 0 ? `<tr><td>Total Returns:</td><td align="right">-${totalReturnAmount.toFixed(2)}</td></tr>` : ""}
          <tr style="font-weight:bold; font-size:15px; border-top:1.5px solid #000;"><td>Total Due:</td><td align="right">Rs.${(subtotal - totalItemDiscount - billDiscount + vatAmount - totalReturnAmount).toFixed(2)}</td></tr>
        </table>
      </div>
    ...
    <SafeAreaView style={{ flex: 1 }} edges={["bottom"]}>
      <View style={styles.container}>
        <View style={styles.headerContainer}>
          ...
          {shopPhone ? (
            <Text style={styles.headerPhone}>{shopPhone}</Text>
          ) : null}
        </View>
                ...
                <View key={idx} style={styles.itemRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.itemName}>
                      {it.Item_description || it.item_name}
                    </Text>
                    <View
                      style={{
                        flexDirection: "row",
                    ...
                    >
                      ...
                      {Number(it.Discount) > 0 && (
                        <View style={styles.itemDiscBadge}>
                          <Text style={styles.itemDiscText}>
                            Disc: {Number(it.Discount).toFixed(2)}
                          </Text>
                        </View>
                      )}
                      {returnedQty > 0 && (
                        <View style={styles.itemReturnBadge}>
                          <Text style={styles.itemReturnText}>
                    ...
                    {returnedQty > 0 && (
                      <Text style={styles.returnedAmountText}>
                        Return Amount: -Rs.{returnedAmount.toFixed(2)}
                      </Text>
                    )}
                  </View>
                  <Text style={styles.itemTotal}>
                    Rs.{Number(it.Net_value).toFixed(2)}
                  </Text>
                </View>
              );
            })}
            ...
            {processing ? (
              ...
              <>
                <MaterialCommunityIcons
                  ...
                  color="white"
                />
                <Text style={styles.btnText}>Print Bill (Receipt)</Text>
              </>
            )}
          </TouchableOpacity>
            ...
            {processing ? (
              ...
              <>
                <MaterialCommunityIcons
                  ...
                  color="white"
                />
                <Text style={styles.btnText}>Print Full Invoice</Text>
              </>
            )}
          </TouchableOpacity>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  itemReturnText: { fontSize: 10, color: "#c2410c", fontWeight: "bold" },
  returnedAmountText: {
    fontSize: 11,
    color: "#c2410c",
    marginTop: 4,
