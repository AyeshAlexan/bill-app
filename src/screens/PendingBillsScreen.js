...
export default function PendingBillsScreen({ navigation }) {
  // Mock data representing the pending bills shown in your image
  const pendingBills = [
    { id: '1', billNo: 'INV-2026-001', shop: 'Kandy City Store', loc: 'Kandy', total: '15,000', due: '15,000', status: 'Pending' },
    { id: '2', billNo: 'INV-2026-002', shop: 'Kandy City Store', loc: 'Kandy', total: '8,500', due: '5,500', status: 'Partial' },
    { id: '3', billNo: 'INV-2026-003', shop: 'Katugastota Super', loc: 'Katugastota', total: '12,000', due: '12,000', status: 'Pending' },
  ];
          ...
          >
            <View style={styles.cardTop}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={styles.date}>2026-01-05</Text>
              </View>
              {/* Status Badge colors change based on status */}
              <View style={[styles.statusBadge, { backgroundColor: item.status === 'Partial' ? '#fef3c7' : '#fee2e2' }]}>
                <Text style={[styles.statusText, { color: item.status === 'Partial' ? '#d97706' : '#ef4444' }]}>
                  {item.status}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.shopRow}>
              ...
              <Text style={styles.shopText}>{item.shop} • {item.loc}</Text>
            </View>
            ...
            <View style={styles.amountRow}>
              <View>
                ...
                <Text style={styles.amtValue}>Rs.{item.total}</Text>
              </View>
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={styles.amtLabel}>Due Amount</Text>
                <Text style={[styles.amtValue, { color: '#ef4444' }]}>Rs.{item.due}</Text>
              </View>
            </View>
          </TouchableOpacity>
        )}
...
const styles = StyleSheet.create({
  ...
  statsBox: { backgroundColor: 'rgba(255,255,255,0.2)', padding: 15, borderRadius: 20 },
